...
import { LogIn, Edit2, Check, X, Eye, DollarSign, Search, Users as UsersIcon } from "lucide-react";
...
function Users() {
  ...
  const saveEdit = async (u: U) => {
    ...
    if (isNaN(newBal)) return toast.error("Invalid balance");
                          ...
                          <div className="flex items-center gap-1.5 justify-center">
                            {editingId === u.id ? (
                              <>
                                <button
                                  onClick={() => saveEdit(u)}
                                  className="h-7 w-7 rounded-lg bg-[rgba(22,219,147,0.12)] border border-[rgba(22,219,147,0.25)] grid place-items-center hover:bg-[rgba(22,219,147,0.2)] transition-colors"
                                >
                              ...
                              <>
                                <button
                                  title="Edit user"
                                  onClick={() => startEdit(u)}
                                  className="h-7 w-7 rounded-lg border border-[rgba(22,219,147,0.12)] grid place-items-center hover:border-[rgba(22,219,147,0.35)] hover:text-[#16DB93] transition-colors text-muted-foreground"
                                >
                                ...
                                <button
                                  ...
                                  className="h-7 w-7 rounded-lg border border-[rgba(22,219,147,0.12)] grid place-items-center hover:border-[rgba(22,219,147,0.35)] hover:text-[#16DB93] transition-colors text-muted-foreground disabled:opacity-40"
                                >
                                  {impersonating === u.id
    ...
    <div className="grid sm:grid-cols-2 gap-4 pt-3">
      <div>
        ...
        {investments.length === 0
          ? <div className="text-xs text-muted-foreground">No investments</div>
          : investments.map((inv) => (
            <div key={inv.id} className="flex justify-between text-xs py-1 border-b border-[rgba(22,219,147,0.05)]">
              ...
              <span className={`${inv.status === "active" ? "text-[#16DB93]" : "text-muted-foreground"}`}>{inv.status}</span>
            </div>
          ))
        }
      </div>
      <div>
        ...
        {deposits.length === 0
          ? <div className="text-xs text-muted-foreground">No deposits</div>
          : deposits.map((dep) => (
            <div key={dep.id} className="flex justify-between text-xs py-1 border-b border-[rgba(22,219,147,0.05)]">
              ...
              <span className={`${dep.status === "approved" ? "text-[#16DB93]" : dep.status === "rejected" ? "text-red-400" : "text-amber-400"}`}>{dep.status}</span>
            </div>
          ))
        }
      </div>
