      ...
      <section className="mx-auto max-w-7xl px-4 py-10">
        <div className="flex gap-2 flex-wrap mb-8 border-b border-white/5 pb-4">
          {[
            ...
            <Link key={l.to} to={l.to} activeOptions={{ exact: true }} activeProps={{ className: "gold-gradient text-black" }}
              className="px-3 py-1.5 rounded-md text-sm border border-white/10 hover:border-[var(--gold)]/50">{l.label}</Link>
          ))}
        </div>
        <Outlet />
      </section>
